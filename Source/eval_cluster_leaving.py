...
def run():
    df = pd.read_csv(r'../DataSet/cluster_with_label.csv')
    load_range = np.load(r'../DataSet/eval_cluster_loadrange.npy').item()
...
def eval_cluster_leaving(df, load_range, cluster=5):
    ...
    result.to_csv(r'../DataSet/leaving_group.csv', index=False,
